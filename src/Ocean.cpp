...
void Ocean::update(float time) {
    ...
    const float islandX = 0.0f;
    const float islandZ = -45.0f;
    const float radX = 85.0f;
    const float radZ = 40.0f;
    
    // Zonas de la playa
    const float falloffStart = 0.85f; 
    const float falloffEnd = 1.6f;
    
    // Multiplicador para calmar el mar abierto (1.0 = normal, 0.4 = muy tranquilo)
    const float seaCalmness = 0.4f; 

    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            float origX = mesh[i][j].s * 18.0f; 
            float origZ = mesh[i][j].t * 18.0f;
            
            float x = origX;
            float z = origZ;
            float height = 0.0f;

            // 1. Océano base calmado
            ...
            // --- FÍSICA DE PLAYA ---
            float dx = origX - islandX;
            float dz = origZ - islandZ;
            float dist = sqrtf((dx * dx) / (radX * radX) + (dz * dz) / (radZ * radZ));

            if (dist < falloffEnd) {
                float factor = (dist - falloffStart) / (falloffEnd - falloffStart);
                if (factor < 0.0f) factor = 0.0f;
                
                // Suavizar el océano base al llegar a la costa
                float smoothFactor = factor * factor * (3.0f - 2.0f * factor);
                height *= smoothFactor;
                
                // CORRECCIÓN: El signo '+' hace que el agua viaje HACIA la orilla
                float shorePhase = dist * 12.0f + time * 3.5f; 
                
                // Ola más suave
                float waveShape = powf(sinf(shorePhase) * 0.5f + 0.5f, 3.0f); 
                float backwash = sinf(shorePhase + PI) * 0.15f; // Resaca sutil

                // Intensidad reducida para un mar tranquilo
                float intensity = (1.0f - factor) * factor * 3.5f;
                height += (waveShape + backwash) * intensity * 0.7f;
                
                // Desplazamiento horizontal correcto (Swash) usando distancia real
                float worldDist = sqrtf(dx * dx + dz * dz) + 0.001f;
                float dirX = -dx / worldDist; // Apunta directo al centro de la isla
                float dirZ = -dz / worldDist;
                
                // Expansión del agua sobre la arena
                float surge = waveShape * intensity * 2.5f; 
                x += dirX * surge;
                z += dirZ * surge;
            }
            // ---------------------------------
...
float Ocean::getHeightAt(float x, float z, float time) const {
    float height = 0.0f;
    const float seaCalmness = 0.4f; // Mismo multiplicador del océano
    ...
    const float radX = 85.0f, radZ = 40.0f;
    float dx = x - islandX;
    float dz = z - islandZ;
    float dist = sqrtf((dx * dx) / (radX * radX) + (dz * dz) / (radZ * radZ));
    
    if (dist < 1.6f) {
        float factor = (dist - 0.85f) / (1.6f - 0.85f);
        if (factor < 0.0f) factor = 0.0f;
        
        float smoothFactor = factor * factor * (3.0f - 2.0f * factor);
        height *= smoothFactor;
        
        // Mismos cálculos de la playa
        float shorePhase = dist * 12.0f + time * 3.5f;
        float waveShape = powf(sinf(shorePhase) * 0.5f + 0.5f, 3.0f); 
        float backwash = sinf(shorePhase + PI) * 0.15f;
        float intensity = (1.0f - factor) * factor * 3.5f;
